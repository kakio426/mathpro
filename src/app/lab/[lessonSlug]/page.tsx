...
export default async function LabLessonPage({ params }: LabPageProps) {
  ...
  return (
    <RouteShell
      eyebrow="수학프로 학습 셸"
      title={lesson ? `${lesson.title} | 수학프로 학습 셸` : "수학프로 학습 셸"}
      description="이 화면은 수학프로의 S4 세션 러너가 들어갈 자리입니다. 지금은 동적 라우트, 레이아웃, 반응형 카드 구조만 확인합니다."
      routeValue={`/lab/${lessonSlug}`}
      highlights={siteConfig.sessionFlow}
      footer={
        <>
          lesson 콘텐츠는 아직 연결되지 않았습니다. 이후 S2에서 콘텐츠 타입 계약을
          고정하고, S4에서 이 페이지에 수학프로 학습 세션 러너를 넣습니다.
        </>
      }
    />
  );
